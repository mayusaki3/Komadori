...
本プロジェクトの単体テストは、原則として「対象モジュールのカバレッジ 100%」を品質ゲートとする。

ただし、実行環境（ブラウザ/DOM）への依存が強く、JSDOM と実ブラウザ差異やイベント挙動の差により、
100% を品質ゲートとして固定するとテストが環境依存・不安定になりやすい領域については例外を認める。

### 例外扱い（100% を必須としない）

- Dock UI（src/dock/panel.ts）
  - DOM 操作・イベント処理などブラウザ依存が中心であり、環境差異を跨いで 100% を保証することが現実的でないため。
  - ただし「外部仕様として必ず維持すべき重要ロジック」はテストで担保する（重要経路は必ず TC を作る）。
- `src/server/main.ts` は実行エントリポイント（glue code）であり単体テスト/カバレッジの対象外（例外）
  - 動作保証は手動スモーク（起動、WS接続、button.click 往復）または結合テストで担保
  - 可能なら `createPanelServer()` のように実体を分離し、main.ts を最小化する

### 原則どおり（100% を目標・品質ゲートとする）

- Control Core（src/control-core.ts）
  - 純粋ロジックとして決定的にテスト可能であり、100% を品質ゲートとする。

- StreamDeck Core（src/streamdeck/core.ts）
  - DOM/ブラウザ依存を持たず、updateKey/sendToCore は注入可能な I/F であるため、純粋ロジックとして 100% を品質ゲートとする。
  - 将来、実 SDK 直結や非決定要素（デバイス状態・タイミング等）を導入する場合は、境界層を別モジュールに分離し、
    境界層のみ例外扱いとする（コアロジックは引き続き 100% を維持する）。
