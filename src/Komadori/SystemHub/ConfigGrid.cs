...
namespace XControls.UI
{
    ...
    public class ConfigGrid : PropertyGrid
    {
        ...
        public class ItemSetting
        {
            ...
            /// <summary>
            /// この項目のタイプを参照したり設定します。特殊なタイプとして"Folder","FullFolder","FullPath","IPAddress","Password"が使用できます。
            /// </summary>
            [
                Category("設定"),
                DefaultValue("string"),
                Description("この項目のタイプを参照したり設定します。特殊なタイプとして\"Folder\",\"FullFolder\",\"FullPath\",\"IPAddress\",\"Password\"が使用できます。")
            ]
        ...
        public class ConfigItem
        {
            ...
            public static string ConvertTextValue(string type, object value)
            {
                ...
                if (type.ToLower().Equals("password")) return value.ToString();
                if (type.ToLower().Equals("folder") || type.ToLower().Equals("fullfolder"))
                {
                    string txt = value.ToString();
                    if (txt.Length > 0 && (!txt.Substring(txt.Length - 1, 1).Equals("\\"))) txt += "\\";
                    return txt;
                }
                if (type.ToLower().Equals("fullpath"))
                {
        ...
        internal class ConfigDataTypeEditor : System.Drawing.Design.UITypeEditor
        {
            ...
            {
                if (valtype == "folder" || valtype == "fullfolder" || valtype == "fullpath" || valtype == "password")
                {
                    // [...]を表示
            ...
            public override object EditValue(System.ComponentModel.ITypeDescriptorContext context, System.IServiceProvider provider, object value)
            {
                ...
                if (valtype == "fullpath")
                {
                    ...
                    if (fileBrowser == null)
                    {
                        ...
                        fileBrowser.Title = "使用するLappet.exeを選択してください。";
                    }
                    // 初期パスを設定
                    if (value is string)
                    {
                        fileBrowser.FileName = value.ToString();
                    }
                    // ファイルを選択
