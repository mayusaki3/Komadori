...
namespace TirolPlaySystem
{
    ...
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            // ライブ用キャプチャ画面
            live = new LiveCapForm();

            // 会場モニター画面
            mon = new MonitorForm();
            
            // タイマー連携
            timRem.LinkageTimerView = new XControls.UI.TimerView[] { live.Timer, mon.Timer, timRap };
        ...
        protected override void OnShown(EventArgs e)
        {
            ...
            // ライブ用キャプチャの設定
            live.Show(this);
            live.Top = this.Top + pnlNico.Top;
            live.Left = this.Left + pnlNico.Left;
        ...
        #region LIVE SELECTOR 制御イベント

        #region btnLiveBlue_Clickイベント

        private void btnLiveBlue_Click(object sender, EventArgs e)
        {
            live.ViewMode = LiveCapForm.ViewModes.None;
        }

        #endregion

        #region btnLiveBar_Clickイベント

        private void btnLiveBar_Click(object sender, EventArgs e)
        {
            live.ViewMode = LiveCapForm.ViewModes.ColorBar;
        }

        #endregion

        #region btnLiveInfo_Clickイベント

        private void btnLiveInfo_Click(object sender, EventArgs e)
        {
            live.ViewMode = LiveCapForm.ViewModes.InfoBar;
        }

        #endregion

        #region btnLiveMon_Clickイベント

        private void btnLiveMon_Click(object sender, EventArgs e)
        {
            live.ViewMode = LiveCapForm.ViewModes.Monitor;
        }
        ...
        {
            if (btnLiveText.IsPushed)
            {
                live.TopMessage = txtTop.Text;
            }
        ...
        private void txtBottom_Validated(object sender, EventArgs e)
        {
            if (btnLiveText.IsPushed)
            {
                live.BottomMessage = txtBottom.Text;
            }
        ...
        #region btnLiveText_Clickイベント

        private void btnLiveText_Click(object sender, EventArgs e)
        {
            if (btnLiveText.IsPushed)
            {
                live.TopMessage = txtTop.Text;
        ...
        private void cmbPlayer_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            txtPlayerInfo.Text = "よみ: " + p.PlayerYomi + "\r\n" +
                                 ...
                                 "よみ: " + p.RobotYomi + "\r\n" +
                                 "エントリ状況: " + p.AnEntry;
            live.PlayerInfo = p.PlayerName + "  ロボット名:" + p.RobotName;
        ...
        private void btnMeaReset_Click(object sender, EventArgs e)
        {
            ...
            // 時間
            float tz = 1.0F;
            //if (GameNo == 2 && players[cmbPlayer.SelectedIndex].EntryInfo.Equals("チロル")) tz = 1.3F;
            txtSet.Text = "";
        ...
        {
            live.Field.WallSetting = field.WallSetting;
            mon.Field.WallSetting = field.WallSetting;
            live.Field.GridText = field.GridText;
            mon.Field.GridText = field.GridText;
            live.Field.GridOn = field.GridOn;
        ...
        #region btnLuppetLunch_Clickイベント

        private void btnLuppetLunch_Click(object sender, EventArgs e)
        {
            if (luppetLunch_proc == false)
            {
                luppetLunch_proc = true;
                string exe = xconf.GetConfigValue("Utility", "LuppetExe");
                try
                {
                    ...
                    foreach (Process p in Process.GetProcesses())
                    {
                        try
                        {
                            ...
                            {
                                int n = p.MainWindowTitle.IndexOf(" - ");
                                if (n >= 0)
                                {
                                    string nam = p.MainWindowTitle.Substring(n + 3);
                                    if (nam.Equals("Left")) left = true;
                                    if (nam.Equals("Right")) right = true;
                                }
                            }
                        }
                        catch { }
                    ...
                    if (right == false) newnam = "Right";
                    if (left == false) newnam = "Left";
                    if (newnam.Length > 0)
                    {
                        Process lup = Process.Start(exe);
                        while (lup.MainWindowHandle == IntPtr.Zero && lup.HasExited == false)
                        {
                            Thread.Sleep(100);
                            lup.Refresh();
                        }
                        while (lup.MainWindowTitle.IndexOf("Config") >= 0 && lup.HasExited == false)
                        {
                            Thread.Sleep(100);
                            lup.Refresh();
                        }
                        if (lup.HasExited == false)
                        {
                            SetWindowText(lup.MainWindowHandle, lup.MainWindowTitle + " - " + newnam);
                        }
                    }
                }
                catch { }
            }
            luppetLunch_proc = false;
        }
        private bool luppetLunch_proc = false;
        ...
        {
            xconf.SetConfigValue("Live", "URL", mon.Browser.CurrentURL.ToString());

            string w;
            w = mon.Browser.ViewNowPosition.Y.ToString() + "," + mon.Browser.ViewNowPosition.X.ToString() + "," + mon.Browser.ZoomRate.ToString() + "," + mon.MaskL.ToString() + "," + mon.MaskR.ToString();
            xconf.SetConfigValue("Live", "LivePosMon", w);
        }
        ...
        #region ロボット写真Drag&Dropイベント

        private void xcam_DragEnter(object sender, DragEventArgs e)
        {
            ...
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                {
                    // 拡張子が画像ファイルかをチェック
                    if (fnam.Substring(fnam.Length - 4).ToLower().Equals(".jpg") ||
                        fnam.Substring(fnam.Length - 4).ToLower().Equals(".png") ||
                        fnam.Substring(fnam.Length - 4).ToLower().Equals(".gif"))
                    {
                ...
                e.Data.GetDataPresent("UniformResourceLocatorW"))
            {
                ...
                // 拡張子が画像ファイルかをチェック
                if (url.Substring(url.Length - 4).ToLower().Equals(".jpg") ||
                    url.Substring(url.Length - 4).ToLower().Equals(".png") ||
                    url.Substring(url.Length - 4).ToLower().Equals(".gif"))
                {
                    e.Effect = DragDropEffects.Copy;
                }
        ...
        private void xcam_DragDrop(object sender, DragEventArgs e)
        {
            // ファイルドロップ
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                e.Data.GetDataPresent("UniformResourceLocatorW"))
            {
                ...
                try
                {
                    ...
                    // 直接Bitmapに設定すると、Discordから貼れないので一旦ファイルを経由する
                    bmp.Save("temp.jpg");
                    xcam.LoadImage("temp.jpg");
                }
                catch(Exception ex)
                {
                }
        ...
        private void SystemInitialize()
        {
            ...
            try
            {
                ...
                // モニタインデックスをモニタ識別に合わせる
                ArrayList slst = new ArrayList();
                int si = 0;
                foreach(Screen scr in Screen.AllScreens)
                {
                    ArrayList s = new ArrayList();
                    s.Add(scr.DeviceName);
                    s.Add(si++);
                    bool ins = false;
                    for (int i = 0; i < slst.Count; i++)
                    {
                        ArrayList sl = (ArrayList)slst[i];
                        if (scr.DeviceName.CompareTo(sl[0].ToString()) < 0)
                        {
                            slst.Insert(i, s);
                            ins = true;
                            break;
                    ...
                    if (ins == false) slst.Add(s);
                }
                monitorScreenNo = (int)((ArrayList)slst[monitorScreenNo])[1];
                ...
                live.ScreenIndex = monitorScreenNo;
            ...
            try
            {
                ...
                for (i = 0; i < pl.Length; i++)
                {
                    ...
                    if (pc.Length >= 6)
                    {
                        ...
                        pi.RobotYomi = pc[3];
                        pi.AnEntry = pc[4];
                        if (pi.AnEntry.Equals("辞退")) pi.PlayerName += " / 辞退";
                        if (pi.AnEntry.Equals("リザーブ")) pi.PlayerName += " / 展示";
                    }
                    players.Add(pi);
                }
                rt = true;
        ...
        {
            string url = xconf.GetConfigValue("Live", "URL");
            string[] posmon = xconf.GetConfigValue("Live", "LivePosMon").Split(new char[] { ',' });
            int x, y, z, l, r;
